...
public class PreviewPage extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request,
		...
		HashMap<String, Product> cartproducts = (HashMap<String, Product>)session.getAttribute("savedOrder");
		HashMap<String, Accessory> cartacc = (HashMap<String, Accessory>)session.getAttribute("accorder");
    	...
    	if(act != null && act.equals("removewarranty")) {
			if(cartproducts == null || cartproducts.isEmpty()) {
				...
				session.setAttribute("savedOrder", cartproducts);
			...
			session.setAttribute("accorder", cartacc);
		...
		response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/preview"));
	}
  ...
  public void doGet(HttpServletRequest request,
    ...
    HttpSession session = request.getSession();
    HashMap<String, Product> orderproducts = (HashMap<String, Product>) session.getAttribute("savedOrder");
	HashMap<String, Accessory> cartacc = (HashMap<String, Accessory>)session.getAttribute("accorder");
	String username = "";
	if(session != null){
    ...
    String contentStr = "<hr><h1 style=\"background-color: #e9e9e9;padding-left: 20px;padding-top: 8px;\"> Preview Order</h1> <hr>";
		  	...
		  	Product c = m.getValue();
	  		 ...
	  		 contentStr = contentStr +
	 			...
	 			"            <div class=\"prod_price_big\"> price : <span class=\"price\">$"+c.getPrice()+"</span></div>"+
				 			 "<form action=\"/csj/preview?act=remove\" method = \"post\">"+
					 			...
					 			"</form>"+
					 			 "<form action=\"/csj/preview?act=removewarranty\" method = \"post\">"+
						 			"  <input type=\"hidden\" name=\"name\" value= \""+c.getName()+"\">"+
						 			"  <button type = \"submit\" value= \"Remove From Cart\" >Remove Warranty</button>"+
						 		"</form>"+
    ...
    if(cartacc != null && !cartacc.isEmpty()) {
		...
		for(Entry<String, Accessory> ma :cartacc.entrySet()){
			...
			contentStr = contentStr +"<div class=\"prod_box\">"+
					...
					"        <div class=\"prod_details_tab\"> "
					 +"<form action=\"/csj/preview?act=removeacc\" method = \"post\">"+
    	...
    	contentStr = contentStr + 	
    			...
    			"              <input type=\"text\" value = \""+username+"\" name=\"name\" class=\"contact_input\" />"+
    			"            </div>"+
    			"            <div class=\"form_row\">"+
    			...
    			"              <input type=\"text\" name=\"cardno\" class=\"contact_input\" />"+
    			"            </div>"+
    						"  <input type=\"hidden\" name=\"ordername\" value= \""+ordername+"\">"+
					 		"  <input type=\"hidden\" name=\"totalprice\" value=\""+totalprice+"\">"+
					 

    			"             <button type=\"submit\" value=\"Check Out\" style=\"margin-left: 275px; padding: 8px; background: burlywood;\" class=\"contact\">Check Out And Make Order</button> "+
    						"</form>"+
    			"          </div>"+
    			"        </div>"+
